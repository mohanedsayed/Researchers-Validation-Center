# Naqqad MVP — Plan 1: Backend Foundation + Upload + Stage-1 Chunking
...
**Goal:** Stand up the Naqqad backend that accepts an anonymous `.docx` upload, encrypts and stores it, runs Stage-1 semantic chunking via an Anthropic API model (Haiku 4.5), persists chunks + detected discipline/language, and securely deletes everything at T+24h.

**Architecture:** FastAPI (Python 3.11) + async SQLAlchemy 2.0 + PostgreSQL 16. Uploaded `.docx` files are AES-256 (Fernet) encrypted and stored in S3-compatible storage (LocalStack in dev). A Celery worker runs the Stage-1 chunker; Celery Beat handles 24h TTL secure deletion. **No accounts** — each session is anonymous and addressed by an unguessable `share_token`. **No Ollama / no local model** — Stage-1 uses the hosted `claude-haiku-4-5` via the official `anthropic` SDK. Stage-2 critique is NOT in this plan.

**Tech Stack:** FastAPI · SQLAlchemy 2.0 async · Alembic · PostgreSQL 16 · Redis 7 · Celery 5 · `anthropic` SDK (Haiku 4.5) · python-docx · boto3 · cryptography · pytest · httpx · pytest-asyncio

**Related:** [MVP design spec](../specs/2026-06-22-naqqad-mvp-design.md) · [chunklib design](../specs/2026-06-18-chunklib-pluggable-chunker-design.md) · adapts [Phase-1 Foundation plan](2026-06-16-naqqad-phase1-foundation.md) (drops Ollama + auth).
...
> **MVP simplification (stated deliberately):** the chunker ships a minimal `chunklib`-aligned provider layer (`LLMProvider` protocol + `AnthropicProvider` + `create_provider(config)`) so the model stays swappable by config. The full pluggable registry (`register_provider`, `OpenAICompatibleProvider`, 100-provider surface) from the chunklib design is **deferred** — YAGNI for validation.

> **Working directory** for all commands: `/Users/mohanedsayed/Researchers-Validation-Center`. Backend commands run from `/Users/mohanedsayed/Researchers-Validation-Center/backend` unless noted.
...
- [ ] **Step 1.3: Create `backend/app/__init__.py`**
...
- [ ] **Step 1.7: Create `docker-compose.yml` at project root**
...
- [ ] **Step 1.8: Create local `.env` from the example and start infra**
...
- [ ] **Step 2.2: Create `backend/app/models/session.py`**
...
- [ ] **Step 2.3: Create `backend/app/models/chunk.py`**
...
- [ ] **Step 2.4: Create `backend/app/models/discipline.py`**
...
- [ ] **Step 2.5: Create `backend/app/models/feedback.py`**
...
- [ ] **Step 3.1: Initialize Alembic inside the backend container**
...
Expected: `backend/alembic/` directory and `backend/alembic.ini` created.

- [ ] **Step 3.2: Replace `backend/alembic/env.py` with the async-compatible version**
...
- [ ] **Step 3.4: Generate and apply the initial migration**
...
Expected output includes: `critique_sessions`, `uploaded_files`, `chunks`, `discipline_instructions`, `feedback`. (No `users` table — anonymous MVP.)
...
## Task 4: Test Harness (conftest)

**Files:**
- Create: `backend/tests/__init__.py` (empty)
- Create: `backend/tests/conftest.py`

> Tests run **locally** against the dockerized `db` and `localstack`. Ensure `docker compose up -d db redis localstack` is running and that a `naqqad_test` database exists (created in Step 4.2).

- [ ] **Step 4.1: Create `backend/tests/__init__.py`**
...
- [ ] **Step 4.2: Create the test database**
...
- [ ] **Step 4.3: Create `backend/tests/conftest.py`**
...
- [ ] **Step 4.4: Verify the harness imports cleanly**
...
Expected: prints a byte count > 0 (a valid `.docx` was generated).
...
## Task 5: File Parser Service (.docx only)
...
- [ ] **Step 5.1: Create `backend/app/services/__init__.py`**
...
- [ ] **Step 5.2: Write the failing test `backend/tests/test_file_parser.py`**
...
## Task 6: S3 Storage Service (AES-256 / Fernet)
...
- [ ] **Step 6.1: Generate a valid Fernet key and set it in `backend/.env`**
...
Copy the output and set it as `FILE_ENCRYPTION_KEY=<output>` in `backend/.env`.

- [ ] **Step 6.2: Write the failing test `backend/tests/test_storage.py`**
...
- [ ] **Step 6.4: Create `backend/app/services/storage.py`**
...
## Task 7: Chunker (chunklib-aligned, Anthropic Haiku)

**Files:**
- Create: `backend/app/services/chunker/__init__.py`
- Create: `backend/app/services/chunker/providers.py`
- Create: `backend/app/services/chunker/service.py`
- Create: `backend/tests/test_chunker.py`

The chunker produces a deterministic baseline from the docx structure (always works, no API), then calls Haiku to (a) group paragraphs into topic-coherent chunks respecting chapter/section boundaries and (b) detect discipline + language. If the Haiku call fails, it falls back to the deterministic baseline.

- [ ] **Step 7.1: Write the failing test `backend/tests/test_chunker.py`**
...
- [ ] **Step 7.3: Create `backend/app/services/chunker/providers.py`**
...
- [ ] **Step 7.4: Create `backend/app/services/chunker/service.py`**
...
- [ ] **Step 7.5: Create `backend/app/services/chunker/__init__.py`**
...
Expected: all 7 tests PASS (deterministic + fake-provider paths; no live API call).
...
## Task 8: Upload + Session API (anonymous)
...
- [ ] **Step 8.1: Create `backend/app/schemas/__init__.py`**
...
- [ ] **Step 8.2: Create `backend/app/schemas/session.py`**
...
- [ ] **Step 8.3: Create `backend/app/schemas/chunk.py`**
...
- [ ] **Step 8.4: Create `backend/app/api/__init__.py`**
...
- [ ] **Step 8.5: Write the failing test `backend/tests/test_upload.py`**
...
Expected: 404 / import errors — routes not implemented yet.

- [ ] **Step 8.7: Create `backend/app/api/upload.py`**
...
- [ ] **Step 8.8: Create `backend/app/api/sessions.py`**
...
> Note: `GET /sessions/{share_token}` and `GET /sessions/{share_token}/chunks` are matched before the upload route's `/upload` only because `/upload` is a literal path on a different method (POST). FastAPI resolves `POST /sessions/upload` to `upload_file` and `GET /sessions/{share_token}` to `get_session` without conflict.

- [ ] **Step 8.9: Run tests — expect the upload/get tests to PASS**
...
Expected: `test_upload_docx_creates_session`, `test_upload_pdf_returns_422`, `test_get_session_by_share_token`, `test_get_session_unknown_token_404` all PASS. (The Celery dispatch is a no-op in tests until Task 9 enables eager mode; the endpoint still returns 202.)
...
## Task 9: Celery Setup, Chunking Task, Cleanup Task
...
- Modify: `backend/tests/test_upload.py` (add eager-mode integration test)

- [ ] **Step 9.1: Create `backend/app/tasks/__init__.py`**
...
- [ ] **Step 9.2: Create `backend/app/tasks/celery_app.py`**
...
- [ ] **Step 9.3: Create `backend/app/tasks/chunking.py`**
...
- [ ] **Step 9.4: Create `backend/app/tasks/cleanup.py`**
...
- [ ] **Step 9.5: Add the eager-mode integration test to `backend/tests/test_upload.py`**
...
Expected: all tests in `test_file_parser.py`, `test_storage.py`, `test_chunker.py`, `test_upload.py` PASS.
...
## Task 10: TTL Cleanup Test + Live Smoke Test

**Files:**
- Create: `backend/tests/test_cleanup.py`

- [ ] **Step 10.1: Write `backend/tests/test_cleanup.py`**
...
- [ ] **Step 10.3: Full suite run**

```bash
python -m pytest tests/ -v --tb=short
```

Expected: all tests across `test_file_parser.py`, `test_storage.py`, `test_chunker.py`, `test_upload.py`, `test_cleanup.py` PASS.

- [ ] **Step 10.4: Start all services, migrate, and smoke-test the live pipeline**
...
- [ ] **Step 10.5: Upload a real docx and watch the session reach `queued`**
...
Expected: status transitions to `queued`, `detected_disciplines` is populated by Haiku, and chunks are returned with chapter/section metadata.
...
- [x] Anonymous `.docx` upload, single file per session → Task 8
- [x] AES-256 encryption at rest → Task 6
- [x] S3 storage (LocalStack) → Task 6
- [x] Stage-1 chunker via hosted API (Haiku 4.5), `chunklib`-aligned provider, default = Anthropic → Task 7
- [x] Deterministic docx structure (python-docx) + Haiku semantic grouping → Tasks 5 + 7
- [x] Auto-detect discipline + language → Task 7 (provider output) + Task 9 (persisted on session)
- [x] Celery async pipeline → Task 9
- [x] Session status state machine (Uploading→Parsing→Queued→…→Expired) → models + Task 9
- [x] 24-hour secure deletion (Celery Beat) → Tasks 9 + 10
- [x] Anonymous shareable handle (`share_token`) + session/chunks read API → Tasks 2 + 8
- [x] `Feedback` model for the validation instrument (written here; endpoint wired in Plan 2 with the report) → Task 2
- [x] No login / no Ollama → Tasks 1, 8 (anonymous), and absence of auth/ollama modules

**Deliberately deferred (later plans / NEXT):**
- Stage-2 critique (Sonnet 4.6) + SSE streaming + jury assembly + DSE auto-apply → Plan 2
- Feedback + report endpoints → Plan 2
- Next.js RTL frontend (3 screens) → Plan 3
- Accounts, billing, PDF, Forensic mode, admin/DSE approval → MVP design §10.2 (NEXT)

**Placeholder scan:** none — every code/test step contains complete content.

**Type consistency:** `ChunkerService.chunk()` returns `ChunkResult` (Task 7) and is consumed as `chunk_result.chunks` / `.detected_language` / `.detected_disciplines` in Task 9. `LLMProvider.chunk_document(outline, paragraphs)` signature matches the fake providers in tests (Task 7) and `AnthropicProvider` (Task 7). `share_token` is defined on the model (Task 2), returned in `SessionResponse` (Task 8), and used as the route key (Task 8).
```
